...
const ToolsAccordion = ({ sections, className = '' }) => {
  ...
  return (
    <div
      className={`rounded-3xl border border-border bg-background-muted/40 p-4 shadow-soft dark:shadow-black/40 ${className}`.trim()}
    >
      ...
      <div className="mt-3 space-y-3">
        {sections.map((section) => {
          ...
          return (
            <div
              key={section.id}
              className="rounded-2xl border border-border/70 bg-background-elevated/80 px-4 py-3"
            >
              <button
                type="button"
